...
#define IDC_ST_COUNT 2013
#define IDC_ST_CONNSTATUS 2014
...
// {7D98E4F4-9CBE-4a90-B31E-AC2D5ABB091B}
static const GUID Project02_CMM = 
{ 0x7d98e4f4, 0x9cbe, 0x4a90, { 0xb3, 0x1e, 0xac, 0x2d, 0x5a, 0xbb, 0x9, 0x1b } };
...
enum {DISCONNECTED, CONNECTED, HOSTING};
			...
			switch(LOWORD(wParam))
			{
				...
				case IDC_BU_HELP:
				{
					MessageBox(0, 
							...
							"Enter name just like with join\n"\
							...
							"The application will automatically close after \n"\
							"being idle for 30 seconds, once a message is sent\n"\
							"the idle counter will be reset to 30 seconds\n"\
							"The counter is in the lower right corner and\n"\
							"does not apply if you are currently hosting\n"\
			...
			if (wParam == TIMERID_CONNECT_COMPLETE)
			{
				if (WAIT_OBJECT_0 == WaitForSingleObject(g_hConnectCompleteEvent, 0))
				{
					...
					else
					{
						...
						SetWindowText(g_hST_ConnStatus, "CONNECTED");
					}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
	...
	while(1)
	{
		...
		if (ConnStatus == CONNECTED)
		{
			...
			sprintf(buffer, "%i", (int)idleTimer);
			if ((int)oldtimer != (int)idleTimer)
				SetWindowText(g_hST_Count, buffer);
			...
			if (idleTimer <= 0) 
			{
				...
				SetWindowText(g_hST_Count, "N/A");
				SetWindowText(g_hST_ConnStatus, "DISCONNECTED");
...
void CreateInterface()
{
	...
	g_hBU_Send = CreateWindow("BUTTON", "Send",  WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
						      380, 300, 60, 28, g_hWnd, (HMENU)IDC_BU_SEND, g_hInstance, 0);

	g_hBU_Sound = CreateWindow("BUTTON", "Sound",  WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
						      380, 330, 60, 28, g_hWnd, (HMENU)IDC_BU_SOUND, g_hInstance, 0);
	...
	g_hST_Count = CreateWindowEx(WS_EX_CLIENTEDGE, "STATIC", "0", WS_CHILD | WS_BORDER | SS_CENTER | WS_VISIBLE, 
		                           580, 330, 40, 28, g_hWnd, (HMENU)IDC_ST_COUNT, g_hInstance, 0);

	g_hST_ConnStatus = CreateWindow("STATIC", "Disconnected", WS_CHILD | WS_BORDER | SS_CENTER | WS_VISIBLE, 
		                           450, 330, 100, 28, g_hWnd, (HMENU)IDC_ST_CONNSTATUS, g_hInstance, 0);
...
HRESULT WINAPI DirectPlayMessageHandler(void*  pUserContext, DWORD MessageID, void* pMsgBuffer)
{
	switch(MessageID)
	{
		...
		{
			ShowText(g_hLB_Output, "Migrate Host");
			...
			if (pHostMigrateMsg->dpnidNewHost == g_dpnidLocalPlayer)
			{
				...
				ConnStatus = HOSTING;
				SetWindowText(g_hST_ConnStatus, "HOSTING");
				SetWindowText(g_hST_Count, "N/A");
		...
		case DPN_MSGID_RECEIVE:
		{
			...
			else if (!strcmp("/idle", (char*)pReceiveMsg->pReceiveData))
			{
				PDPNMSG_DESTROY_PLAYER pDestroyMsg = (PDPNMSG_DESTROY_PLAYER)pMsgBuffer;
				pDestroyMsg->dwReason = DPNDESTROYPLAYERREASON_HOSTDESTROYEDPLAYER;
				pDestroyMsg->dpnidPlayer = pReceiveMsg->dpnidSender;
				...
				for (i = 0; i < MAX_PLAYERS; i++)
				{
					...
					{
						if (g_playerInfo[i].dpnidPlayer == pReceiveMsg->dpnidSender)
							break;
					}
				}
				char msg[256];
				sprintf("%i was dropped due to idle time", g_playerInfo[i].name);
				ShowText(g_hLB_Output, msg);
				DestroyPlayer(pUserContext, pMsgBuffer);
...
HRESULT HostGame()
{
	...
	dnAppDesc.guidApplication = Project02_CMM;
...
HRESULT JoinGame()
{
	...
	dpnAppDesc.guidApplication = Project02_CMM;
...
HRESULT SendChatMessage(int player, char *message)
{
	...
	if (player == -2)
	{
		...
		{
			if (g_playerInfo[i].dpnidPlayer != g_dpnidLocalPlayer)
				g_pDP->SendTo(g_playerInfo[i].dpnidPlayer, &bufferDesc, 1, 0, 0, &hAsync, 0);
		}
	}
	else if (player == -1)
