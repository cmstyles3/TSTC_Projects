...
int InitD3D(char *classname, char *windowname, int width, int height, bool windowed, D3DDEVTYPE Devicetype)
{
	...
	wc.cbClsExtra    = 0;
	wc.cbWndExtra    = 0;
	wc.hInstance     = hinstance_app;
	wc.hIcon         = LoadIcon(hinstance_app, MAKEINTRESOURCE(ICON_ID));
	wc.hCursor       = LoadCursor(0, IDC_ARROW);
	wc.hbrBackground = (HBRUSH)GetStockObject(WHITE_BRUSH);
	wc.lpszMenuName  = 0;
	wc.lpszClassName = classname;
	wc.cbSize        = sizeof(WNDCLASSEX);
	wc.hIconSm       = LoadIcon(hinstance_app, MAKEINTRESOURCE(ICON_ID));
	...
	if (!(hwnd = CreateWindowEx(0, classname, windowname, WS_OVERLAPPEDWINDOW | WS_VISIBLE, 0, 0, width, height, 0, 0, hinstance_app, 0)))
	{
