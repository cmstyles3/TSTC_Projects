...
bool CInput::InitDI()
{
	if (FAILED(DirectInput8Create(hinstance_app, DIRECTINPUT_VERSION, IID_IDirectInput8, (void **)&m_pDI, NULL)))
		return false;
...
bool CInput::InitKB()
{
	...
	if (m_pKBDev->SetCooperativeLevel(hwnd_app, DISCL_NONEXCLUSIVE | DISCL_BACKGROUND) != DI_OK)
		return false;
...
bool CInput::InitMouse()
{
	...
	if (m_pMouseDev->SetCooperativeLevel(hwnd_app, DISCL_EXCLUSIVE | DISCL_FOREGROUND) != DI_OK)
		return false;
