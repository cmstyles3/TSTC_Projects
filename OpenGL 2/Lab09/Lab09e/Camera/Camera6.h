...
class Camera{
  private:
//	Point3 eye;					// made this public in camera6.h
	Vector3 u, v, n, up;
...
void Camera::slide(float delU,float delV,float delN)
{
	...
	//if(eye.y < 0.0) eye.y = 0.1;				// added 8-2-07
	setModelViewMatrix();
}
...
{
	/*eye.y += amt;*/
	float cs = cos(3.14159265/180 * amt);
	float sn = sin(3.14159265/180 * amt);
	Vector3 t(n); // remember old n
	v.set(cs*t.x - sn*u.x, cs*t.y - sn*u.y, cs*t.z - sn*u.z);
	n.set(sn*t.x + cs*u.x, sn*t.y + cs*u.y, sn*t.z + cs*u.z);
	//v = n.cross(u);	
	setModelViewMatrix();
}
...
//	float radius = sqrt(pow(eye.x - look.x, 2.0) + pow(eye.z - look.z, 2.0));
	float radius = sqrt(pow(eye.x - look.x, 2) + pow(eye.z - look.z, 2));
	float x1, x2, z1, z2, a;
	...
	look.x = look.x + x2 - x1;
	look.z = look.z + z2 - z1;
//	a = sqrt(pow(x2 - x1, 2.0) + pow(z2 - z1, 2.0));
