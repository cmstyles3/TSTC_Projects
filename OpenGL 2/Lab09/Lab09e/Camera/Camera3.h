...
{
// rotate the camera around its own axis (v-axis) (first part is same as yaw)
	float cs = cos(3.14159265/180 * angle);
	float sn = sin(3.14159265/180 * angle);
	Vector3 oldn(n); // remember old n
	Vector3 oldv(v); // remember old v
	n.set(cs*oldn.x - sn*u.x, cs*oldn.y - sn*u.y, cs*oldn.z - sn*u.z);
	u.set(sn*oldn.x + cs*u.x, sn*oldn.y + cs*u.y, sn*oldn.z + cs*u.z);
	v.set(cs*oldv.z - sn*u.z , cs*oldv.y - sn*u.y, cs*oldv.x - sn*u.x);	// adjust for tilt of v-axis during rotation
	setModelViewMatrix();
/*
	float pitchangle, rise, csq, c, bsq, b, asq, a;
//	pitchangle = atan((eye.x - look.x) / eye.y - look.y ) * (180.0 / 3.14159265);
//	csq = pow(eye.x - look.x, 2.0) + pow(eye.y - look.y, 2.0);
//	c = sqrt(csq);
	a = eye.x - look.x;
	b = eye.y - look.y;
	rise = (2.0 * b) * (angle / 180.0);
//	n.y -= rise;
//	u.y -= rise;
	setModelViewMatrix();
*/
