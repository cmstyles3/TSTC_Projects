...
void Input::Update()
{
	...
	m_cursorPos.x += MouseDX();
	m_cursorPos.y += MouseDY();

	if (m_cursorPos.x < 0) m_cursorPos.x = 0;
	else if (m_cursorPos.x > g_engine->GetWidth() - 1) m_cursorPos.x = g_engine->GetWidth() - 1;
	if (m_cursorPos.y < 0) m_cursorPos.y = 0;
	else if (m_cursorPos.y > g_engine->GetHeight() - 1) m_cursorPos.y = g_engine->GetHeight() - 1;
...
D3DXVECTOR3 Input::GetCursorPos()
{
	m_cursorPos.z = 0.0f;
	return m_cursorPos;
}
