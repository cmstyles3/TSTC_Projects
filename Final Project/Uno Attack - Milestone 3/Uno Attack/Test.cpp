...
void Test::Load()
{
	...
	D3DXCreateTextureFromFileNonPow2(g_engine->GetDevice(), "Assets/Textures/cards.dds", &m_tex);
...
void Test::Update(float dt)
{
	...
	{
		m_someoneWon = true;
		strcpy(m_winningMsg, "You have defeated the AI opponent!!!");
	}
	if (m_aiHand.size() <= 0) 
	{
		m_someoneWon = true;
		strcpy(m_winningMsg, "The AI opponent has won, better luck next time");
	}
...
void Test::Draw()
{
	...
	if (m_someoneWon) 
	{
		g_font->Draw(300, 200, D3DCOLOR_XRGB(rand()%255, rand()%255, 255), m_winningMsg);
	}
...
{
	for (int i = m_cardDeck.size()-1; i > 0; i--)
	{
		int randcard = rand()%i;
