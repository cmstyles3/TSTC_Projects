...
enum {EXPLODE_WAV, FIREWORK_WAV, NIGHT_WAV, CHEERING_WAV};
...
int GameInit()
{
	...
	// NEW ///////////////////////////////////////////////////////////////////
	// INITIALIZE DIRECTINPUT AND DEVICES
	if (!InitDirectInput())
	{
		return(0);
	}
	...
	// INITIALIZE DIRECTSOUND AND LOAD WAVES INTO SECONDARY BUFFERS
	if (!InitDirectSound())
	{
		return(0);
	}
	...
	{
		LoadWAV("explode.wav", 0);
		LoadWAV("firework.wav", 1);
		LoadWAV("night.wav", 2);
		LoadWAV("cheering.wav", 3);
	}
	////////////////////////////////////////////////////////////////////
...
int GameMain()
{
	...
	//READ MOUSE DATA
	//MOUSE BUTTONS WILL CONTROL CHEERING
	ReadMouse();
	...
	//READ JOYSTICK DATA
	//copy string to buffer to test input
	
	ReadJoystick();
	if (joystate.lX < -200)
		strcpy(buffer, "stick moved left");
	if (joystate.lX > 200)
		strcpy(buffer, "stick moved right");
	if (joystate.lY < -200)
		strcpy(buffer, "stick moved up");
	if (joystate.lY > 200)
		strcpy(buffer, "stick moved down");

	if (joystate.rgbButtons[0])
		strcpy(buffer, "button press");
