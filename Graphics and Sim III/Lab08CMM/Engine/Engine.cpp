...
Engine::Engine(EngineSetup *setup)
{
	...
	m_network = new Network(m_setup->guid, m_setup->NetworkMessageHandler);
	...
	if (m_loaded)
	{
		...
		while(1)
		{
			...
			else
			{
				...
				if (!DeviceLost())
				{	
					// if there is a current state call its update function
					if (m_currentState != NULL)
						m_currentState->Update(dt);

					else if (m_setup->Update != NULL)
						m_setup->Update(dt);

					if (m_currentState != NULL)
						GetDevice()->Clear(0, 0, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, m_currentState->GetColor(), 1.0f, 0);

					else GetDevice()->Clear(0, 0, D3DCLEAR_TARGET, m_setup->clearColor, 1.0f, 0);
