...
MAIN::MAIN(Setup *setup) : D3DApp(setup)
{
	...
	MeshScale(m_level2Meshs[0], 15.0f);
	m_level2Meshs[0]->LockVertexBuffer(0, (void**)&vertices);
	...
	MeshScale(m_level2Meshs[1], 5.0f);
	m_level2Meshs[1]->LockVertexBuffer(0, (void**)&vertices);
...
void MAIN::UpdateLevel1(float dt)
{
	...
	if (!m_paused)
	{
		...
		//FLOATING OBJECT
		m_position[1].x += m_speed * dt;

		if (m_position[1].x > 200) 
		{
			m_position[1].x = -200;
			m_position[1].y = rand()%300 - 150;
		}

		// update bounding box
		m_aabb[1].c = m_position[1];
...
void MAIN::DrawLevel1()
{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		m_level1Meshs[i]->DrawSubset(0);
		if (m_showBoundingVolume)
		{
			g_device->SetRenderState(D3DRS_FILLMODE, D3DFILL_WIREFRAME);
			m_level1Boxes[i]->DrawSubset(0);
...
void MAIN::UpdateLevel2(float dt)
{
	...
	if (!m_paused)
	{
		...
		//FLOATING OBJECT
		m_position[1].y -= m_speed * dt;

		if (m_position[1].y < -150) 
		{
			m_position[1].y = 150;
			m_position[1].x = rand()%400 - 200;
		}

		// update bounding box
		m_sphere[1].center = m_position[1];

		if (TestSphereSphere(m_sphere[0], m_sphere[1]))
		{
			m_collisionCount++;
			if (m_collisionCount >= 10)
				m_currState = INIT_LEVEL3;//DISPLAY_START;
			else
			{
				m_speed *= 2;
				m_position[1].y = -260;
...
void MAIN::DrawLevel2()
{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		D3DXMatrixTranslation(&T, m_position[i].x, m_position[i].y, m_position[i].z);
		g_device->SetTransform(D3DTS_WORLD, &T);
		m_level2Meshs[i]->DrawSubset(0);
		if (m_showBoundingVolume)
		{
			g_device->SetRenderState(D3DRS_FILLMODE, D3DFILL_WIREFRAME);
			m_level2Spheres[i]->DrawSubset(0);
			g_device->SetRenderState(D3DRS_FILLMODE, D3DFILL_SOLID);
		}
	}
...
void MAIN::UpdateLevel3(float dt)
{
	...
	if (!m_paused)
	{
		...
		m_obb[1].c = m_position[1];

		if (TestOBBOBB(m_obb[0], m_obb[1]))
		{
			m_collisionCount++;
			if (m_collisionCount >= 10)
				m_currState = INIT_RESULTS;//DISPLAY_START;
			else
			{
				m_speed *= 2;
				m_position[1].y = -260;
...
void MAIN::DrawLevel3()
{
	...
	for (int i = 0; i < 2; i++)
	{
		m_object3D[i].SetWorldMatrix(m_position[i].x, m_position[i].y, m_position[i].z);
		m_level1Meshs[i]->DrawSubset(0);
		if (m_showBoundingVolume)
		{
			g_device->SetRenderState(D3DRS_FILLMODE, D3DFILL_WIREFRAME);
